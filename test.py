...
df_train, df_trash = train_test_split(df_train_all,test_size=16296, random_state=0)
...
n_fold = 5
n_params = 20
alpha_range = np.logspace(-3, -2, n_params)
...
alpha_range = np.linspace(0.001, 0.1, n_params)
...
for i in range(len(training_sizes)):
    ...
    for k, (train_idx, test_idx) in enumerate(kf.split(X_train_lc, y_train_lc)):
        ...
        # Learn the model
        w_hat = fns.optimalSolution(X_train_std, y_val_train, alpha_opt_quad)
...
plt.xlabel('size')
plt.xlim([training_sizes[0], training_sizes[-1]])
plt.title('Learning curves for quadratic features')
plt.legend()
plt.show()

# Increase training set size
df_train, df_trash = train_test_split(df_train_all,test_size=14846, random_state=0)
...
# Tune alpha for Linear RLSR
alpha_range = np.logspace(-5, -4, n_params)
for i in range(len(alpha_range)):
    alpha = alpha_range[i]
    for k, (train_idx, test_idx) in enumerate(kf.split(X_train, y_train)):
        X_val_train, y_val_train = X_train[train_idx], y_train[train_idx]
